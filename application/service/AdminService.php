...
class AdminService
{
     ...
     */
    public static function AdminList($params = [], $admin_id)
    {
        ...
        // 获取管理员列表
        $data = Db::name('Admin')->where($where)->whereOr('id', '=', $admin_id)->whereOr('parent_id', '=', $admin_id)->field($field)->order($order_by)->limit($m, $n)->select();
        if(!empty($data))
        {
            foreach($data as &$v)
            {
     ...
     */
    public static function AdminTotal($where, $admin_id)
    {
        return (int) Db::name('Admin')->where($where)->whereOr('id', '=', $admin_id)->whereOr('parent_id', '=', $admin_id)->count();
    }
